...
uniform sampler3D m_VXGI_VoxelMap;
uniform vec3 m_VXGI_GridMin;
uniform vec3 m_VXGI_GridMax;
uniform int m_VXGI_GridSize;

#define TRACING_VOXEL_MAP m_VXGI_VoxelMap
...
void main(){
    ...
    for (int i = 0; i < VXGI_LIGHT_DATA_SIZE; i += COMPONENTS_PER_LIGHT) {
        vec4 lightColor = readVector4(i);
        int lightType = int(lightColor.a);
        #ifdef VXGI_SHADOWS
        ...
        vec4 lightData1 = readVector4(i + 4);     
        vec4 lightDir;
        vec3 lightVec;            
        ...
        if (lightType > 1.0) {
        #endif
            fallOff =  computeSpotFalloff(readVector4(i + 8), lightVec);
        #if __VERSION__ >= 110
        }
        #endif
    ...
    vec3 indirect = approximateSurfaceIndirectVXGI(wPosition, normalize(mix(wNormal, normal, 0.0)), normalize(wPosition - g_CameraPosition), Metallic, Roughness, specularColor.rgb, 1.0);
    gl_FragColor.rgb += mix(indirect, indirect * diffuseColor.rgb, 1.0);
